...
14. **Receipt Settings (Prompt A)**: `/settings` page (owner-only) with 4 tabs — Business Identity (name, address, logo URL, colors), Receipt Layout (toggles for logo/pickup date/garment list/payment history/T&C, header color), Terms & Conditions (editable textarea), and Team & Sites. Settings stored in `businessSettings` table (one row per userId). Backend: `GET /api/settings`, `PUT /api/settings`. Frontend: `client/src/lib/receipt-settings.ts` holds `ReceiptSettings` interface + `DEFAULT_SETTINGS`; `client/src/lib/receipt.ts` `generateDepositReceipt` and `generatePaymentReceipt` accept settings param. Orders and payments pages fetch settings and pass them to receipt generators.
15. **Multi-Site Management (Prompt B)**: `organisations`, `sites`, `siteMembers`, `siteInvitations` tables. Each user is auto-migrated to an org+site on login/register via `migrateToMultiSite`. `/api/sites` CRUD (owner-only), `/api/sites/:id/members` CRUD, `/api/invitations` (create/accept/delete), `/api/auth/switch-site` (POST, stores `currentSiteId` in session). `useAuth()` exposes `currentSite`, `allSites`, `userRole`, `isOwner`, `switchSite`, `canAccess(page)`. `LayoutShell` has a `SiteSwitcher` dropdown for owners and a static site badge for managers/operators; nav items filtered by role. Settings page Team tab manages sites + sends invitations. Accept-invitation page at `/join/:token`.
16. **Consolidated Dashboard (B6)**: When an owner selects "All Sites" (currentSite = null via SiteSwitcher), the dashboard shows an "All Sites — Consolidated View" banner + a "Sites Overview" card grid listing every site with name, city, member count, and active status. When viewing a specific site, a compact "Viewing: {name}" banner is shown. Backend `GET /api/analytics/dashboard` reads `currentSiteId` from session (or falls back to DB) and passes `allSites=true` to storage, which returns `sitesOverview`, `siteCount`, and `isAllSites` fields.
