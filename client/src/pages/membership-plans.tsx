...
export default function MembershipPlansPage() {
  ...
  return <div className="space-y-6 page-fade-in">
    ...
    <Tabs defaultValue="plans"><TabsList><TabsTrigger value="plans">Plans</TabsTrigger><TabsTrigger value="subscribers">Subscribers</TabsTrigger><TabsTrigger value="revenue">Revenue</TabsTrigger></TabsList></Tabs>
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{plans.map(plan => <Card key={plan.id} className="overflow-hidden"><CardHeader className="pb-3"><div className="flex items-start justify-between"><div><CardTitle>{plan.name}</CardTitle><p className="mt-1 text-sm text-muted-foreground">{Number(plan.recurringPrice).toLocaleString()} FCFA / {t(plan.billingCycle)}</p></div><Badge variant={plan.status === "active" ? "default" : "secondary"}>{plan.status}</Badge></div></CardHeader><CardContent className="space-y-4"><div className="flex items-center gap-2 text-sm"><Users className="h-4 w-4" />{plan.subscriberCount} {t("active_subscribers").toLowerCase()}</div><div className="flex flex-wrap gap-1">{plan.services.map(s=><Badge key={s.id} variant="outline">{s.name}</Badge>)}</div><div className="flex gap-2"><Button size="sm" variant="outline" onClick={()=>edit(plan)}>Edit</Button><Button size="sm" variant="outline" onClick={()=>duplicate.mutate(plan.id)}><Copy className="h-4 w-4" /></Button><Button size="sm" variant="ghost" disabled={plan.status==="archived"} onClick={()=>archive.mutate(plan.id)}><MoreHorizontal className="h-4 w-4" /></Button></div></CardContent></Card>)}</div>
