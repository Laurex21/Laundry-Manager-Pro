      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="sm:max-w-[520px] max-h-[90vh] overflow-y-auto">
            ...
            <ServiceForm onSuccess={() => setOpen(false)} categories={allCategories} />
          </DialogContent>
        </Dialog>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {services?.map((service) => (
            <Card key={service.id} className="group hover:shadow-md transition-all duration-300 border-border/50" data-testid={`card-service-${service.id}`}>
...
function ServiceForm({ onSuccess, categories }: { onSuccess: () => void; categories: string[] }) {
  ...
  function onSubmit(data: InsertService) {
    ...
    mutate(payload as InsertService, {
      onSuccess: () => {
        form.reset();
        onSuccess();
      }
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-2">
        ...
        <Button type="submit" className="w-full mt-2" disabled={isPending} data-testid="button-save-service">
          {isPending ? t('saving') + "..." : t('save_service')}
        </Button>
      </form>
    </Form>
  );
