...
assert.match(routesSource, /revokeInvitation\(invitationId, org\.id, tx\)/);
assert.match(routesSource, /if \(!revoked\) return res\.status\(404\)/);
assert.match(storageSource, /revokeInvitation\(id: number, organisationId: number, executor: any = db\)[\s\S]{0,500}eq\(siteInvitations\.organisationId, organisationId\)/);
...
assert.match(authStorageSource, /consumeValidPasswordResetToken\(tokenHash: string, executor: any = db\)[\s\S]{0,500}isNull\(passwordResetTokens\.usedAt\)[\s\S]{0,250}returning\(\)/);
assert.match(authStorageSource, /revokeUserSessions\(userId: string, executor: any = db\)[\s\S]{0,150}delete\(sessions\)[\s\S]{0,150}sessions\.sess.*userId/);
...
assert.match(routesSource, /upsertSettings\(userId, parsed\.data, tx\)[\s\S]{0,800}settings\.updated[\s\S]{0,300}, tx\)/);
assert.match(routesSource, /updateSiteMemberRole\([^\n]+, tx\)[\s\S]{0,500}staff\.role_updated[\s\S]{0,300}, tx\)/);
assert.match(routesSource, /removeSiteMember\([^\n]+, tx\)[\s\S]{0,500}staff\.removed[\s\S]{0,300}, tx\)/);
assert.match(routesSource, /createInvitation\([^\n]+, tx\)[\s\S]{0,500}invitation\.created[\s\S]{0,300}, tx\)/);
assert.match(routesSource, /revokeInvitation\(invitationId, org\.id, tx\)[\s\S]{0,500}invitation\.revoked[\s\S]{0,300}, tx\)/);
