...
const pdfRender = readFileSync(join(root, "server/lib/pdf-render.ts"), "utf8");
...
assert.match(receipt, /doc\.documentElement\.classList\.add\("pdf-export"\)/);
assert.match(receipt, /function downloadReceiptPdfFromHtml/);
assert.match(receipt, /fetch\("\/api\/receipts\/render-pdf"/);
assert.match(receipt, /stripNoPrintElements\(html\)/);
assert.match(routes, /app\.post\("\/api\/receipts\/render-pdf"/);
assert.match(pdfRender, /import\("puppeteer-core"\)/);
assert.match(pdfRender, /emulateMediaType\("print"\)/);
assert.match(pdfRender, /const scale = Math\.max/);
assert.match(pdfRender, /printBackground: true/);
assert.match(receipt, /downloadReceiptPdfFromHtml\(html, `deposit-receipt-order-\$\{displayOrderId\}\.pdf`\)/);
assert.match(receipt, /downloadReceiptPdfFromHtml\(html, `payment-receipt-order-\$\{orderId\}\.pdf`\)/);
...
assert.doesNotMatch(receipt, /deposit-receipt-order-\$\{displayOrderId\}\.html/);
assert.doesNotMatch(receipt, /payment-receipt-order-\$\{orderId\}\.html/);
assert.match(packageJson, /"puppeteer-core"/);
