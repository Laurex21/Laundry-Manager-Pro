...
const customerDetail = readFileSync(join(root, "client/src/pages/customer-detail.tsx"), "utf8");
...
assert.match(routes, /discount: z\.coerce\.number\(\)\.min\(0\)\.max\(1_000_000_000\)\.default\(0\)/);
...
assert.match(component, /navigate\(`\/orders\?correct=\$\{order\.id\}`\)/);
assert.match(ordersPage, /data-testid="correction-reason-section"/);
assert.match(ordersPage, /data-testid="order-correction-form-loading"/);
assert.match(ordersPage, /data-testid="order-correction-form-error"/);
assert.match(ordersPage, /refetchCorrectionOrder/);
assert.match(ordersPage, /\/api\/orders\/\$\{correctionOrder\.id\}\/correct/);
assert.match(ordersPage, /discount: formattedData\.discount/);
assert.match(ordersPage, /discountPct:/);
assert.match(service, /discountPct > 0[\s\S]{0,220}normalizeDecimalInput\(input\.discount\)/);
assert.match(service, /Discount must be between zero and the order subtotal/);
assert.match(translations, /order_correction_check_failed:/);
assert.match(translations, /create_corrected_order: "Corriger la commande"/);
assert.match(translations, /order_correction_load_failed:/);
...
assert.match(customerDetail, /order\.discountAmount \?\? order\.discount \?\? 0/);
assert.match(customerDetail, /text-order-discount-/);
