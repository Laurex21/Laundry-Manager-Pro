...
export function generateSubscriberReceiptHTML(data: any, thermalWidth?: 58 | 80) {
  ...
  *{box-sizing:border-box}body{margin:0;background:${thermal ? "#fff" : "#f3f4f6"};font-family:Arial,sans-serif;color:#171717}main{width:${width};max-width:100%;margin:auto;background:#fff;padding:${thermal ? "4mm" : "14mm"};font-size:${thermal ? "9px" : "14px"}}
  ...
  <section class="section"><h2 class="section-title">Situation de l’abonnement</h2><div class="billing"><div class="billing-row"><span>Montant de l’abonnement</span><strong>${money(plan.recurringPrice)}</strong></div><div class="billing-row"><span>Statut du paiement</span><strong class="payment-status ${paymentStatus.className}">${paymentStatus.label}</strong></div></div></section>
