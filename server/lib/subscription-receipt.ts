...
export function generateSubscriberReceiptHTML(data: any, thermalWidth?: 58 | 80) {
  ...
  <section style="border:${thermal ? "1px dashed #000" : "1px solid #bfdbfe"};padding:12px"><div style="display:flex;justify-content:space-between"><span>Montant total commande</span><strong>${money(order.totalAmount)}</strong></div><div style="display:flex;justify-content:space-between"><span>✓ Couvert par l’abonnement</span><strong>- ${money(coverage.coveredAmount)}</strong></div><div style="display:flex;justify-content:space-between;font-size:1.15em;margin-top:8px"><span>Montant à payer</span><strong>${money(coverage.extraAmount)}</strong></div></section>
