...
assert.match(ordersPage, /const \[discountPercentageInput, setDiscountPercentageInput\] = useState\("0"\)/);
assert.match(ordersPage, /value=\{discountPercentageInput\}/);
assert.match(ordersPage, /Math\.min\(100, Math\.max\(0, percentage\)\)/);
assert.match(ordersPage, /rawValue === "" \? "" : String\(clampedPercentage\)/);
assert.match(ordersPage, /form\.setValue\("discountPct", clampedPercentage/);
assert.match(ordersPage, /setDiscountPercentageInput\("0"\)/);
