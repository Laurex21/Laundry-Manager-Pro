...
export async function registerRoutes(httpServer: Server, app: Express): Promise<Server> {
  ...
  // Switch site
  app.post("/api/receipts/pdf", isAuthenticated, async (req, res) => {
    try {
      const { html } = req.body as { html?: string };
      if (!html || typeof html !== "string") {
        return res.status(400).json({ message: "Missing html body" });
      }
      const fs = await import("fs");
      // Find the real Chromium executable: check env var, standard paths, then
      // scan /nix/store for dirs named exactly `<hash>-chromium-<version>`.
      const nixCandidates: string[] = [];
      try {
        const nixStore = "/nix/store";
        for (const entry of fs.readdirSync(nixStore)) {
          // Match e.g. "zi4f80l169xlmivz8vja8wlphq74qqk0-chromium-125.0.6422.141"
          // but NOT "hunspell-dict-*-chromium-*", "*-chromium-unwrapped-*", "*-chromium-bsu-*"
          if (/^[a-z0-9]+-chromium-\d+\.\d+\.\d+/.test(entry) && !entry.includes("unwrapped") && !entry.includes("sandbox")) {
            nixCandidates.push(`${nixStore}/${entry}/bin/chromium`);
          }
        }
      } catch { /* ignore */ }
      ...
      try {
        ...
        const pxToMm = (px: number) => Math.ceil((px / 96) * 25.4);
        const heightMm = Math.max(pxToMm(bodyHeight) + 10, 120);
      ...
      console.error("PDF receipt render error:", err);
